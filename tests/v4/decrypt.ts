...
test('it decrypts a message using a key', (t: TestContext) => {
    const decrypted = decrypt(key, TOKEN);
    t.assert.deepStrictEqual(decrypted, MESSAGE_OBJ);
});

test('it decrypts a message using a key and a footer', (t: TestContext) => {
    const decrypted = decrypt(key, TOKEN_WITH_FOOTER);
    t.assert.deepStrictEqual(decrypted, Object.assign(MESSAGE_OBJ, {
        footer: 'test'
    }));
});

test('it decrypts a message using a key and a footer and an assertion', (t: TestContext) => {
...
test('it throws if token does not match tag', (t: TestContext) => {
    try {
        const decrypted = decrypt(key, TOKEN_WITH_ASSERTION);
        t.assert.fail('Should have thrown');
    } catch (e) {
        t.assert.strictEqual(e instanceof PasetoDecryptionFailed, true);
    }
});
